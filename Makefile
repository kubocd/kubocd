...
crds: manifests kustomize ## Generate crds file into helm chart
	$(KUSTOMIZE) build config/crd -o helm/kubocd/crds/crds.yaml
...
.PHONY: chart-yaml
chart-yaml: ## Generate the helm/kcd-controller/Chart.yaml
	echo "$$CHART_YAML" >./helm/kubocd/Chart.yaml

.PHONY: chart
chart: chart-yaml crds ## Build and push helm chart
	cd ./helm && helm package -d ./../tmp kubocd && helm push ./../tmp/kubocd-${HELM_VERSION}.tgz oci://${HELM_DOCKER_REPO}

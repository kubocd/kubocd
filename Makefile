...
APP_VERSION := v0.4.0-snapshot
HELM_VERSION := v0.4.0-snapshot
...
verify-tool-versions: ## Verify that .tool-versions and go.mod are in sync (MAJOR.MINOR comparison).
	@ENVTEST_K8S_VERSION=$(ENVTEST_K8S_VERSION) bash hack/verify-tool-versions.sh

.PHONY: verify-envtest-version
verify-envtest-version: verify-tool-versions ## Verify active envtest assets match the expected Kubernetes version.
	@ENVTEST_K8S_VERSION=$(ENVTEST_K8S_VERSION) bash hack/verify-envtest-version.sh
...
.PHONY: build
build:  display manifests generate fmt vet build-kubocd ## Build kubocd binaries with dependencies

.PHONY: build-kubocd
build-kubocd: ## Build kubocd binary.
	CGO_ENABLED=0 go build -ldflags '-X kubocd/internal/global.Version=$(APP_VERSION) -X kubocd/internal/global.BuildTs=$(BUILD_TS)' -o bin/kubocd cmd/kubocd/main.go
...
.PHONY: docker-build
docker-build: check-registry ## Build docker image with the manager.
...
docker-buildx: check-registry display  ## Build and push docker image with cross-platform support
	...
	- $(CONTAINER_TOOL) buildx build --builder kubocd-builder --push --platform=$(PLATFORMS) --build-arg VERSION=$(APP_VERSION) --build-arg BUILD_TS=$(BUILD_TS) --tag $(IMG_REPO):$(APP_VERSION) -f Dockerfile .

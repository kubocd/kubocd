...
func desugarConnectionRef(path string, node map[string]interface{}) (map[string]interface{}, bool, error) {
	contract, err := contractOf(path, node)
	if err != nil {
	...
	marker := map[string]interface{}{"contract": contract}
...
// same name (or connection type) is carried by a Connection AND a ClusterConnection.
...
func contractOf(path string, node map[string]interface{}) (string, error) {
	rawContract, ok := node["contract"]
	if !ok {
		return "", fmt.Errorf("node '%s': 'contract' is required", path)
	}
	contract, ok := rawContract.(string)
	if !ok || contract == "" {
		return "", fmt.Errorf("node '%s': 'contract' must be a non-empty string", path)
	}
	return contract, nil
}
...
func collectConnectionDecls(node map[string]interface{}, path []string, required bool, inArray bool, isContext bool, decls *[]ConnectionDecl) error {
	...
	if marker, ok := node[MarkerConnectionRef]; ok {
		...
		contract, _ := markerMap["contract"].(string)

...
func refDecl(path []string, contract string, required bool) kuboschema.ConnectionDecl {
	return kuboschema.ConnectionDecl{Path: path, Contract: contract, Required: required}
}
...
func TestGenerateScalarRef(t *testing.T) {
	...
	if len(gen) != 1 || gen[0].Alias != "parameters.metadataDb" ||
		gen[0].ConnectionRef.Name != "kcd-postgres-superset" ||
		gen[0].ConnectionRef.Namespace != "okdp" || gen[0].Optional {
...
func TestGenerateRefKindConnection(t *testing.T) {
	...
	if len(gen) != 1 || gen[0].Kind != kv1alpha1.KindConnection ||
		gen[0].ConnectionRef.Namespace != "okdp" {
		t.Fatalf("unexpected generated input: %+v", gen)
	}
}
...
func TestGenerateRefKindClusterConnection(t *testing.T) {
	...
	if gen[0].ConnectionRef.Namespace != "" {
		t.Errorf("a ClusterConnection ref must not carry a namespace, got '%s'", gen[0].ConnectionRef.Namespace)
	}
}
...
func TestGenerateRefWithoutKind(t *testing.T) {
	...
	if len(gen) != 1 || gen[0].Kind != "" || gen[0].ConnectionRef.Namespace != "okdp" {
...
func TestGenerateContextRefKind(t *testing.T) {
	...
	if len(gen) != 1 || gen[0].Alias != "context.platform.oidc" ||
		gen[0].Kind != kv1alpha1.KindClusterConnection || gen[0].ConnectionRef.Namespace != "" {
		t.Fatalf("unexpected generated input: %+v", gen)
	}
}
...
func TestAliasCollisionWithStanza(t *testing.T) {
	...
	stanza.Contract = "database-server"
...
func TestRenderRefDefaults(t *testing.T) {
	decl := kuboschema.ConnectionDecl{
		Path: []string{"oidc"}, Contract: "oidc", Required: true,
